...
function registerEvents() {
  ...
  el.swatches.addEventListener('click', e => {
    ...
    // Teks footer preview menyesuaikan warna frame
    const dark = isColorDark(state.frameColor);
    el.stripFoot.querySelector('.strip-foot-brand').style.color = dark ? '#888' : 'rgba(0,0,0,0.45)';
    el.stripFoot.querySelector('.strip-foot-date').style.color  = dark ? '#aaa' : 'rgba(0,0,0,0.55)';
  });
}
...
async function uploadAndGenerateQR() {
  ...
  try {
    ...
    setStatus('✅ Foto siap! Klik tombol kirim untuk mencetak.');
...
async function buildGridCanvas() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const x = PAD + col * (SLOT_W + GAP), y = LOGO_H + PAD + row * (SLOT_H + GAP);
    dc.fillStyle = '#ffffff'; dc.fillRect(x - 6, y - 6, SLOT_W + 12, SLOT_H + 12);
    const img   = await loadImage(state.captured[i]);
    const cropW = img.width, cropH = img.width * (SLOT_H / SLOT_W);
    const cropX = 0, cropY = (img.height - cropH) / 2;
...
async function buildVerticalCanvas() {
  ...
  for (let i = 0; i < state.captured.length; i++) {
    ...
    const cropW = img.width, cropH = img.width * (SH / SW);
    const cropX = 0, cropY = (img.height - cropH) / 2;
  ...
  dc.fillStyle = getCanvasTextColor(dark ? 0.55 : 0.4);
  dc.fillText('✦ SNAPBOOTH ✦', TW / 2, TH - FH + 30);
  dc.font      = 'bold 13px sans-serif';
  dc.fillStyle = getCanvasTextColor(dark ? 0.65 : 0.5);
...
/**
 * Cek apakah warna hex termasuk gelap (luminansi < 0.4)
 */
...
/**
 * Warna teks canvas — putih untuk frame gelap, hitam untuk frame terang
 */
